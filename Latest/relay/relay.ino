...
#define MY_ADDRESS        101   // The ID of this node (must be different for every node on network)
...
#define FREQUENCY      433.0 //RF69_868MHZ // RF69_915MHZ
  ...
  if (rf69_manager.available())
  {
    ...
    // Choose me
      if (buf[0] == chooseMePayload.id) {
        ...
        listening_to_node = 1 * from;
        if (Serial) { Serial.print("Listening now to node: "); Serial.println(listening_to_node); }
        
        chooseMePayload.value = getResponseCode(from);
        // Send a reply back to the originator client
        
        if (!rf69_manager.sendtoWait((uint8_t *)&chooseMePayload, sizeof(chooseMePayload), from)) {
          if (Serial) Serial.println("Sending failed (no ack)");}
        else {
          if (Serial) Serial.println("Sending success");
        }
        // Actual data
      } else if (buf[0] == payload.id) {
       
        payload = *(Payload*)buf; //assume radio.DATA actually contains our struct and not something else
...
int getResponseCode(int sender) {
  //if (!ble.isConnected()) return 102;
  if (listening_to_node == sender) return 200;
  return 101;
}
...
void printDebugInfo() {
  ...
  Serial.println("------------------------------------");
  Serial.print("I am node: "); Serial.println(MY_ADDRESS);
...
/*
    if (rf69_manager.sendtoWait((uint8_t *) &upd, sizeof(upd), DEST_ADDRESS)) {
